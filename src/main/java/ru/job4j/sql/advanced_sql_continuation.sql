...
-- Подзапросы используются с IN, NOT IN, ANY, ALL, EXISTS или NOT EXISTS. Они выполняются первыми.
-- ORDER BY - нельзя использовать с подзапросами. Разделяются на скалярные, многострочные и коррелированные. Работают медленее JOINS*.

-- UNION при объединении таблиц дубликаты удаляются, выполняя DISTINCT для удаления дубликатов.
-- UNION ALL дубликаты не удаляются, выполняется быстрее так как не использует DISTINCT.
-- EXPECT - отображается только тот результат первой таблицы, которого нету во второй таблице.
-- INTERSECT - отображает результат который пересекается в одной и второй таблице.
-- - UNION объединяет результаты двух или более запросов в один результирующий набор, который включает все строки,
-- которые принадлежат всем запросам в объединении. При выполнении UNION данные из двух таблиц будут собраны в один столбец с одинаковым типом данных;
-- - JOIN позволяет извлекать данные из двух и более таблиц на основе логических связей между таблицами.
-- В результатах выборки все столбцы будут показаны отдельно, т.е. столбцы из объединяемых таблиц будут расставлены рядом друг с другом, слева направо.
-- При использовании операторов множеств количество столбцов в каждом из запросов SELECT должно быть одинаковым.
-- При этом в качестве имен столбцов будут выбраны те, которые указаны для них в первом из запросов SELECT.
...
-- Вывод всех уникальных названий произведений из таблиц movie и book (т.е фильмы, которые сняты не по книге, и книги без экранизации)
(SELECT title
FROM book
EXCEPT
SELECT name
...
-- Стоимость запросов в Postgres EXPLAIN.
-- Единица стоимости по умолчанию привязана к одной последовательной странице, которая прочитана со стоимость 1.0 единица (seq_page_cost).
-- Каждая обработанная строка добавляет 0.01 единицы (cpu_tuple_cost), а каждая непоследовательно прочитанная страница добавляет 4.0 единицы (random_page_cost).
-- Подобных констант гораздо больше, каждую из них можно изменить.
-- Пример стоимости запросов EXPLAIN SELECT * FROM users ORDER BY username;
EXPLAIN ANALYSE (SELECT title
    FROM book
    EXCEPT
    SELECT name
    FROM movie)
    ...
    SELECT title
    FROM book) LIMIT 1;
-- EXPLAIN ANALYZE выполняет фактический запрос, отображает в милисекундах время выполнения. Лимит устанавливается если будет 1 строк в таблице.
-- CREATE INDEX book_title_index ON book(name);
-- EXPLAIN ANALYZE SELECT * FROM book ORDER BY name;
-- ANALYZE book; - Эта команда собирает статистическую информацию о содержимом таблиц в БД и сохраняет результаты в системном каталоге pg_statistic.
...
-- 1. Затраты в планах запросов – это оценки Postgres того, сколько времени займет SQL-запрос в произвольных единицах.
-- 2. Планировщик запросов выбирает план с наименьшей стоимостью на основе некоторых настраиваемых констант, а также собранной статистики.
-- 3. Очень важно помочь планировщику более точно оценить затраты на выполнение запросов. Таким образом можно помочь сделать правильный выбор и повысить эффективность запросов.